...
def signed_request(handler_func, verifier_func, signer_func, query_str: str, key_group: str, sig: str, **kwargs):
    ...
    if sig != None and verifier_func(query_str, int(sig, 16), key_group):
        ...
        ret_code = 403
    answer = f'{answer}#{hex(signer_func(answer, "afcs"))[2:]}'
...
def key_kos_exchange_handler(id: str, n: str, e: str):
    ...
    Returns:
        str: Строка с открытым ключом AFCS.
    """
    n, e = str(int(n, 16)), str(int(e, 16))
    key_entity = get_entity_by_key(UavPublicKeys, id)
    if key_entity == None:
        save_public_key(n, e, f'kos{id}')
    afcs_n, afcs_e = get_key('afcs', private=False)
    str_to_send = f'$Key: {hex(afcs_n)[2:]} {hex(afcs_e)[2:]}'
    return str_to_send
...
def key_ms_exchange_handler(id: str):
    ...
    if f'ms{id}' not in loaded_keys:
        generate_keys(AFCS_KEY_SIZE, key_group)
    ...
    if key_entity == None:
        ...
        key_entity.e = e
        commit_changes()
    afcs_key_pk = get_key('afcs', private=True).publickey()
    afcs_n, afcs_e = afcs_key_pk.n, afcs_key_pk.e
    str_to_send = f'$Key: {hex(afcs_n)[2:]} {hex(afcs_e)[2:]}'
    return str_to_send
